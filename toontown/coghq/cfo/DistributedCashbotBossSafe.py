...
class DistributedCashbotBossSafe(DistributedCashbotBossObject.DistributedCashbotBossObject):
    ...
    def __init__(self, cr):
        ...
        self.toMagnetSoundInterval = Parallel(
            SoundInterval(self.flyToMagnetSfx, duration=GeneralCFOGlobals.CashbotBossToMagnetTime, node=self),
            Sequence(
                Wait(GeneralCFOGlobals.CashbotBossToMagnetTime - 0.02),
    ...
    def _doDebug(self, _=None):
        self.boss.safeStatesDebug(
            doId=self.doId, content="(Client) state change %s ---> %s" % (self.oldState, self.newState)
        )
    ...
    def announceGenerate(self):
        ...
        if self.index == 0:
            ...
            self.collisionNode.setIntoCollideMask(PieBitmask | WallBitmask)
            self.collisionNode.setFromCollideMask(PieBitmask)
