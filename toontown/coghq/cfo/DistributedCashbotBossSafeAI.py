...
from . import CraneLeagueGlobals, DistributedCashbotBossObjectAI, GeneralCFOGlobals
import math

from .DistributedCashbotBossSideCraneAI import DistributedCashbotBossSideCraneAI
...
class DistributedCashbotBossSafeAI(DistributedCashbotBossObjectAI.DistributedCashbotBossObjectAI):
    ...
    def _doDebug(self, _=None):
        self.boss.safeStatesDebug(
            doId=self.doId, content="(Server) state change %s ---> %s" % (self.oldState, self.newState)
        )
    ...
    def hitBoss(self, impact, craneId):
        ...
        if self.state not in ("Dropped", "Grabbed"):
            return
    ...
    def requestGrab(self):
        avId = self.air.getAvatarIdFromSender()
        if self.state not in ("Grabbed", "Off"):
            craneId, objectId = self.__getCraneAndObject(avId)
            crane = simbase.air.doId2do.get(craneId)
            if crane and craneId != 0 and objectId == 0:
